...
async function limitedResponseBody(
  ...
  if (contentLength !== null) {
    ...
    if (!Number.isSafeInteger(declared) || declared < 0 || declared > maximumBytes) {
      throw new GatecrashError(`${label} is unexpectedly large.`);
    }
  }
  if (response.body === null) {
    throw new GatecrashError(`${label} was empty.`);
  }
...
async function downloadAsset(
  ...
  let response: Response;
  try {
    response = await fetcher(asset.browser_download_url, {
      headers: {'user-agent': 'gatecrash-updater'},
      redirect: 'follow',
      signal: AbortSignal.timeout(60_000),
    });
  } catch {
    throw new GatecrashError(`Could not download the ${label.toLowerCase()} from GitHub.`);
  }
  if (!response.ok) {
    throw new GatecrashError(`${label} download failed with HTTP ${response.status}.`);
  }
  return limitedResponseBody(response, maximumBytes, label);
}

export function expectedChecksum(contents: string, archiveName: string): string {
  ...
  throw new GatecrashError(`SHA256SUMS does not contain ${archiveName}.`);
}
...
export async function downloadVerifiedRelease(
  ...
  try {
    ...
    throw new GatecrashError('The release checksum file is not valid UTF-8.');
  }
  ...
  if (actual !== expected) {
    throw new GatecrashError('The downloaded Gatecrash archive failed SHA-256 verification.', {
      hint: 'Nothing was installed. Try the update again later.',
    });
  }
  return archive;
}
